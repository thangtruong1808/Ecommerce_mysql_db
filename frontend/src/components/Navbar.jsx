...
const Navbar = () => {
  ...
  return (
    <nav ref={navRef} className="bg-blue-600 shadow-md relative">
        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:flex space-x-4 items-center">
            ...
            <button
              ...
              className="px-3 py-2 rounded-md text-sm font-medium inline-flex items-center transition-colors text-white hover:text-blue-100"
            ...
            <Link
              ...
              className={`px-3 py-2 rounded-md text-sm font-medium inline-flex items-center transition-colors ${
                isActive("/clearance")
                  ...
                  : "bg-red-500 text-white hover:text-white/80 font-semibold"
              }`}
            >
              <FaTag className="mr-1.5" />{" "}
              <span className="text-md">Clearance</span>
            </Link>
            <Link
              to="/cart"
              className={`px-3 py-2 rounded-md text-sm font-medium relative inline-flex items-center transition-colors ${
                isActive("/cart")
                  ? "text-white bg-blue-700 font-semibold"
                  : "text-white hover:text-blue-100"
              }`}
            ...
            {isAuthenticated ? (
              <div className="relative" ref={userMenuRef}>
                <button
                  ...
                  className="flex items-center space-x-2 text-white hover:text-blue-100 px-3 py-2 rounded-md text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-400"
                >
                  <FaUserCircle className="text-xl" /> <span>{user?.name}</span>{" "}
                  ...
                  <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg border border-gray-200 z-50">
                    ...
                    {user?.role === "admin" && (
                      <Link
                        to="/admin/dashboard"
                        onClick={closeUserMenu}
                        className={`flex items-center space-x-2 px-4 py-3 text-sm border-t border-gray-100 transition-colors ${
                          isActive("/admin")
                            ? "bg-blue-50 text-blue-600 font-semibold"
                            : "text-gray-700 hover:bg-gray-100"
                        }`}
                    ...
                    <button
                      ...
                      className="w-full flex items-center space-x-2 px-4 py-3 text-sm text-red-600 hover:bg-red-50 border-t border-gray-100"
                    >
          ...
          <div className="flex md:hidden items-center space-x-2">
            <Link
              to="/cart"
              className="text-white hover:text-blue-100 px-2 py-2 relative"
            >
      ...
      <div className="hidden md:block border-t border-blue-700 relative">
        ...
        {hoveredCategoryId &&
          (() => {
            ...
            return (
              <div
                className="fixed left-0 right-0 bg-white shadow-2xl border-t border-gray-100 z-40"
                style={{ top: `${megaMenuTop}px` }}
                onMouseEnter={() => handleCategoryHover(hoveredCategoryId)}
                onMouseLeave={handleCategoryLeave}
              >
                      ...
                      {subcategories.map((sub) => {
                        ...
                        return (
                          <div key={sub.id} className="space-y-3">
                            <Link
                              to={`/products?category=${hoveredCategory.id}&subcategory=${sub.id}`}
                          ...
                          <div className="relative h-full p-6 flex flex-col justify-between text-white">
                            <div>
                              ...
                              <h4 className="text-2xl font-bold mb-2">
                                {hoveredCategory.name} Clearance
                              </h4>
                              <p className="text-sm opacity-90">
                                Explore exclusive deals on{" "}
                                {hoveredCategory.name.toLowerCase()} items
                              </p>
                            </div>
      ...
      {isMobileMenuOpen && (
        <div
          className="md:hidden fixed inset-0 bg-black bg-opacity-50 z-50 top-16"
          ...
          >
            <div className="px-4 py-6 space-y-4">
              <Link
                to="/"
                onClick={closeMobileMenu}
                className={`flex items-center space-x-2 py-2 text-base font-medium transition-colors ${
                  isActive("/", true)
                    ? "text-blue-600 bg-blue-50 px-2 rounded font-semibold"
              ...
              <div className="border-t border-gray-200 pt-4">
                ...
                >
                  ...
                  <span className="text-xs">{isMegaOpen ? "▲" : "▼"}</span>
                </button>
                <button
                  type="button"
                  onClick={openFilterDrawer}
                  className="w-full text-left py-2 text-base font-medium text-gray-900 hover:text-gray-800 flex items-center space-x-2"
                >
                  <FaSearch /> <span>Filters</span>
                </button>
                {isMegaOpen && (
              ...
              <Link
                ...
                className={`flex items-center space-x-2 py-2 text-base font-medium transition-colors ${
                  isActive("/clearance")
                    ? "text-red-900 bg-red-200 px-2 rounded font-bold"
                    : "text-red-800 hover:text-red-900"
                }`}
              >
                <FaTag /> <span>Clearance</span>
              </Link>
              {isAuthenticated ? (
                <div className="border-t border-gray-200 pt-4 space-y-2">
                  ...
                  <button
                    ...
                    className="w-full flex items-center space-x-2 px-2 py-2 text-red-600 hover:bg-red-50"
                  >
                    <FaSignOutAlt className="text-red-400" />{" "}
                    <span>Logout</span>
                  </button>
                ...
                <div className="border-t border-gray-200 pt-4">
                  <Link
                    ...
                    className={`flex items-center space-x-2 py-2 text-base font-medium transition-colors ${
                      isActive("/login")
                        ? "text-gray-900 bg-yellow-100 px-2 rounded font-semibold"
                        : "text-gray-900 hover:text-gray-800"
                    }`}
                  >
