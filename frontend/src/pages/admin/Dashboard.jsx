...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // Fetch all dashboard data in parallel. Promise.all will reject if any of the promises reject.
      // The axios interceptor will handle 401s, refresh the token, and retry.
      // If the retry succeeds, Promise.all will not see an error.
      // If the refresh fails, the rejection will be caught by the `catch` block below.
      const [
        overviewData,
        salesByCategoryData,
      ...
      // If Promise.all succeeds, all data is valid and can be set.
      if (overviewData) setStats(overviewData);
      if (salesByCategoryData) setCategoryData(salesByCategoryData);
      if (customerInsightsData?.growthData)
        setCustomerGrowthData(customerInsightsData.growthData);
      if (topProductsData) setTopProducts(topProductsData);
      if (recentActivityData) setRecentActivities(recentActivityData);
      if (performanceMetricsData) setPerformanceMetrics(performanceMetricsData);

      setLastUpdated(new Date());
    } catch (error) {
      // This block will catch any error that the axios interceptor couldn't handle (e.g., final refresh failure).
      // We only show a toast notification if it's not a 401 error that is being handled by the refresh mechanism.
      if (!shouldSuppress401Toast(error)) {
        toast.error(
          error.response?.data?.message ||
            "Failed to load dashboard data. The page may display incomplete information."
        );
      }
    } finally {
      setLoading(false);
      ...
      <div className="max-w-full mx-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-evenly sm:items-center mb-2">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center mb-2">
            ...
            <h1 className="text-3xl font-bold text-gray-900 text-center mr-2 mt-2 sm:mt-0">
              Admin Dashboard
            </h1>

            {lastUpdated && (
              <p className="text-sm text-gray-500 flex items-center">
                <FaClock className="mr-1" /> Updated:{" "}
            ...
            {stats?.recentOrders && stats.recentOrders.length > 0 ? (
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {stats.recentOrders.map((order, orderIndex) => {
                  const orderStatus = order.is_delivered
                    ? "delivered"
                    : order.is_paid
                    ? "paid"
                    : "pending";
                ...
                {recentActivities.length > 0 ? (
                  recentActivities.map((activity, index) => (
                    <div
                      ...
                      className="flex items-start space-x-2 pb-2 border-b border-gray-200 last:border-0"
                    >
