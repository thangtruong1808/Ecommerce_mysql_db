...
const VoucherManagement = () => {
  ...
  usePageTitle(loading ? "Loading..." : "Voucher Management");
  const {
  ...
  const handleBulkStatus = async (isActive) => {
    try {
      ...
      await axios.post(
        "/api/admin/vouchers/bulk-update",
        {
          voucherIds: ids,
          isActive,
        },
      ...
      toast.success(
        `${ids.length} voucher(s) ${isActive ? "activated" : "deactivated"}`
      );
      clear();
      fetchVouchers();
    } catch (error) {
                ...
                {vouchers.length === 0 && !loading ? (
                  ...
                  vouchers.map((voucher, index) => (
                    <VoucherTableRow
                      ...
                      onToggle={toggle}
                      onEdit={() =>
                        crud.setEditModal({ isOpen: true, entity: voucher })
                      }
                      onDelete={() =>
                        crud.setDeleteModal({ isOpen: true, entity: voucher })
                      }
                    />
                  ))
                )}
