...
const Profile = () => {
  ...
  useEffect(() => {
    if (user) reset({ name: user.name || '', email: user.email || '', newPassword: '', confirmPassword: '' })
    fetchAddresses()
  }, [user, reset])
  ...
  /** Format date for display @param {string} dateString - Date string to format @returns {string} Formatted date string @author Thang Truong @date 2025-12-12 */
  const formatDate = (dateString) => {
    if (!dateString) return 'N/A'
    return new Date(dateString).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })
  }

  /** Handle profile form submission @param {Object} data - Form data (name, email) @author Thang Truong @date 2025-12-12 */
  ...
  /** Handle password change form submission @param {Object} data - Form data (newPassword, confirmPassword) @author Thang Truong @date 2025-12-12 */
  const onPasswordSubmit = async (data) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        reset({ ...watch(), newPassword: '', confirmPassword: '' })
      } else {
  ...
  const onAddressSubmit = async (data) => {
    ...
    try {
      ...
      if (editingAddress) {
        await axios.put(`/api/auth/addresses/${editingAddress.id}`, addressData)
          ...
          {isEditing ? (
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="flex space-x-4 pt-4 border-t">
                ...
                <button type="button" onClick={() => { setIsEditing(false); reset({ name: user?.name || '', email: user?.email || '' }) }} className="px-6 py-2 bg-gray-300 text-gray-700 rounded-md hover:bg-gray-400 flex items-center space-x-2 transition" disabled={isLoading}><FaTimes /> <span>Cancel</span></button>
              </div>
            </form>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-1"><label className="block text-sm font-medium text-gray-500">Full Name</label><p className="text-lg text-gray-900">{user?.name || 'N/A'}</p></div>
              <div className="space-y-1"><label className="block text-sm font-medium text-gray-500">Email Address</label><p className="text-lg text-gray-900">{user?.email || 'N/A'}</p></div>
              <div className="space-y-1"><label className="block text-sm font-medium text-gray-500">Account Role</label><span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800 capitalize"><FaShieldAlt className="mr-1" /> {user?.role || 'user'}</span></div>
              <div className="space-y-1"><label className="block text-sm font-medium text-gray-500">Member Since</label><p className="text-lg text-gray-900 flex items-center"><FaCalendarAlt className="mr-2 text-gray-400" /> {formatDate(user?.created_at)}</p></div>
            </div>
          )}
        </div>
        <div className="bg-white shadow-md rounded-lg p-6">
