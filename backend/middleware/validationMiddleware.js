...
export const validateBulkOperation = (req, res, next) => {
  ...
  if (!ids.every((id) => Number.isInteger(parseInt(id)) && parseInt(id) > 0)) {
    return res
      .status(400)
      .json({ message: "All IDs must be positive integers" });
  }
...
export const validateOrderStatusTransition = async (req, res, next) => {
  try {
    ...
    if (
      !status ||
      !["pending", "processing", "paid", "delivered", "cancelled"].includes(
        status
      )
    ) {
      return res.status(400).json({
        message:
          "Invalid status. Must be: pending, processing, paid, delivered, or cancelled",
      });
    }
    ...
    const currentStatus = order.is_delivered
      ? "delivered"
      : order.is_paid
      ? "paid"
      : "pending";
...
export const validateUserDeletion = async (req, res, next) => {
  try {
    ...
    if (orders[0].count > 0) {
      return res.status(400).json({
        message:
          "Cannot delete user that has orders. Consider deactivating instead.",
      });
    }
